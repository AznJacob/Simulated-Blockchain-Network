...
def choice_one():
    ...
    unconfirmed_tx_text = 'Unconfirmed_T_A.txt'
    if payer == 1:
        unconfirmed_balance_text = 'Unconfirmed_balance_A1.txt'
        payer_tx = "A0000001"
    else:
        unconfirmed_balance_text = 'Unconfirmed_balance_A2.txt'
        payer_tx = "A0000002"
    ...
    if tx_amount_with_fee > unconfirmed_balance:
        ...
        clientSocket.sendto(encoded_tx, serverPortF1)


def print_balances():
    print("Balances: ")
    with open('balance_A.txt', 'r') as rf:
        name = rf.read(8)
...
def print_unconfirmed_transactions():
    with open('Unconfirmed_T_A.txt', 'r') as rf:
        text = rf.read()
        if text != '':
            print(f"Client A's unconfirmed transactions: {text}")
        else:
            print("There are no unconfirmed transactions yet for A")


def print_confirmed_transactions():
    with open('Confirmed_T_A.txt', 'r') as rf:
        text = rf.read()
        if text != '':
            print(f"Client A's confirmed transactions: {text}")
        else:
            print("There are no confirmed transactions yet for A")
...
def update_balances():
    with open("Confirmed_balance_A1.txt", "r") as rfc, open("Unconfirmed_balance_A1.txt", "r") as rfu:
        confirmed_balance_a1 = rfc.read()
        unconfirmed_balance_a1 = rfu.read()
        balances_a1 = "A0000001" + unconfirmed_balance_a1 + confirmed_balance_a1
    with open("Confirmed_balance_A2.txt", "r") as rfc, open("Unconfirmed_balance_A2.txt", "r") as rfu:
        confirmed_balance_a2 = rfc.read()
        unconfirmed_balance_a2 = rfu.read()
        balances_a2 = "A0000002" + unconfirmed_balance_a2 + confirmed_balance_a2
    all_balances = balances_a1 + balances_a2
    with open("balance_A.txt", "w") as wf:
        wf.write(all_balances)


def reset_everything():
    print("Resetting everything related to client A...")
    ...
    with open("Confirmed_balance_A2.txt", "w") as wfc, open("Unconfirmed_balance_A2.txt", "w") as wfu:
        wfc.write("000003E8")
        wfu.write("000003E8")
    with open("Unconfirmed_T_A.txt", "w") as wfa:
        wfa.write('')
    with open("Confirmed_T_A.txt", "w") as wft:
        wft.write('')
    with open("Temp_T_F1.txt", "w") as wff:
        wff.write('')
